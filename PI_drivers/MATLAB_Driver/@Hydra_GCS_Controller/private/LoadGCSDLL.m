...
if (isWindows)
    ...
    dllFileNameStub = 'PI_HydraPollux_GCS2_DLL';
...
if ~(exist ( dllFullFileName, 'file' ) == 2)
    error ( ['PI MATLAB Driver requires a software component which is not installed on the PC. ' ...
        'Please run "PI_GCS_Library_PI_HydraPollux_GCS2_DLL_Setup.exe" ' ...
...
if(~libisloaded(c.libalias))
    ...
    try
        piMatlabDriverPath = strrep ( mfilename ( 'fullpath' ), '@Hydra_GCS_Controller\private\LoadGCSDLL', '' );
        cd (piMatlabDriverPath);
        
        warning('off', 'all'); % If not disabled missing linux only functions will rise a warning
        if ( matlabIs64bit )
            [notfound,warnings] = loadlibrary (dllFullFileName, @PI_HydraPollux_GCS2_DLL_prototype_x64, 'alias', c.libalias);
        else
            [notfound,warnings] = loadlibrary (dllFullFileName, @PI_HydraPollux_GCS2_DLL_prototype, 'alias', c.libalias);
        end
        warning('on', 'all');
