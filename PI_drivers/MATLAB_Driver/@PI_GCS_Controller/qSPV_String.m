function [value] = qSPV_String(c,memType,containerUnit,functionUnit,parameter) 
%   DESCRIPTION
%   
...
%
%   SYNTAX
%        [value] = PIdevice.qSPV_String(memType,containerUnit,functionUnit,parameter)
% 
%   INPUT
...
%
%   OUTPUT
%       value (char array)                   Memory for the answer
%      
...
if(any(strcmp(functionName,c.dllfunctions)))
    value =  blanks(10001);
	try
		[bRet,~,~,~,~,value] = calllib(c.libalias,functionName,c.ID,memType,containerUnit,functionUnit,parameter,value,10000);
		if(bRet==0)
			iError = GetError(c);
