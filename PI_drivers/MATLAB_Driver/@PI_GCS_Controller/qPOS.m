function [dValues] = qPOS(c,szAxes)
%   DESCRIPTION
%   Get the current positions of szAxes. 
%
%   For controllers with GCS 2.00 syntax:
%       If no position sensor is present in your system, the response to qPOS() is not meaningful.
%       To request the current position of input signal channels (sensors) in physical units, use qTSP() instead. 
%
%   For controllers with GCS 3.0 syntax:
%       szAxes can only include one axis or None for all axes
%
%   SYNTAX
%       [dValues] = PIdevice.qPOS(szAxes) 
% 
%   INPUT
%       szAxes (char array)             string with axes
%
%   OUTPUT
%       [dValues] (double array)        array to receive the current positions of the axes 
%      
%   PI MATLAB Class Library Version 1.5.0
%   COPYRIGHT © PHYSIKINSTRUMENTE (PI) GMBH U. CO. KG, support-software@pi.ws
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if(nargin==1)
    dValues = CallGcs_InAxesOrNone_OutDoubleArray_Function(c, mfilename);
else
    dValues = CallGcs_InAxesOrNone_OutDoubleArray_Function(c, mfilename, szAxes);
end    
